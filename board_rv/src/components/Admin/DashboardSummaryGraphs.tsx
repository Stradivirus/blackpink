...
const planColors: Record<"베이직"|"프로"|"엔터프라이즈", string> = {
  베이직: "#764ba2",
  프로: "#e67e22",
  엔터프라이즈: "#3498db"
};
...
const DashboardSummaryGraphs: React.FC = () => {
  ...
  // 색상 배열 및 summary 데이터 선언 (return문 위에서)
  const osColors: Record<string, string> = {
    Windows: "#3498db",
    Linux: "#27ae60",
    macOS: "#9b59b6",
  ...
  const bizChartData = {
    ...
    datasets: [
      ...(["베이직", "프로", "엔터프라이즈"] as const).map((plan) => ({
        label: plan,
        data: data.biz[plan],
  ...
  const devChartData = {
    ...
    datasets: devOsKeys.map((os) => ({
      label: os,
      data: data.dev[os].map((v: number) => Math.round(v)),
      ...
      {summary && (
        <div style={{ display: "flex", justifyContent: "center", gap: 24, margin: "40px 0 32px 0" }}>
          <div style={summaryCardStyle}>
            <div style={summaryLabelStyle}>가입자 수</div>
            <div style={summaryNumberStyle}>{summary.totalMembers}</div>
          ...
          <div style={summaryCardStyle}>
            <div style={summaryLabelStyle}>진행 중인 프로젝트</div>
            <div style={summaryNumberStyle}>{summary.totalProjects}</div>
            <div style={summarySubStyle}>
          ...
          <div style={summaryCardStyle}>
            <div style={summaryLabelStyle}>보안사고</div>
            <div style={summaryNumberStyle}>{summary.totalIncidents}</div>
            <div style={summarySubStyle}>
